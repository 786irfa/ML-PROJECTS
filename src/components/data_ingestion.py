...
class DataIngestion:
    ...
    def Intiate_data_ingestion(self):
        ...
        try:
            dp=pd.read_csv('Notebook/data/Stud.csv')
            logging.info("read the datasetas dataframe")
            os.makedirs(os.path.dirname(self.ingestion_config.train_data_path,),exist_ok=True)
            dp.to_csv(self.ingestion_config.train_data_path,index=False,header=True)
            logging.info("train test split intiated")
            train_set,test_set=train_test_split(dp,test_size=0.2,random_state=42)
